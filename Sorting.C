...
*/
	...
	fh_infile1.close();
    	...
    	/*
	T->Add("~/rootfiles/test_91.root");
    	T->Add("~/rootfiles/test_92.root");
    	T->Add("~/rootfiles/test_94.root");
	*/
	
      
	//parameter to write in the output root file for center of mass for each gem
	//std::vector < float > xposition;
	//std::vector < float > yposition;
	//Double_t* xposition=new Double_t[NDet];
	//Double_t* yposition=new Double_t[NDet];
	Double_t xposition[NDet],yposition[NDet];
	Double_t qdcchargeDN;
	Double_t qdcchargeUP;
	...
	for(int detnumber=0;detnumber<NDet;detnumber++)
	{
        ...
        if(isOut)
	{
		TFile* rootoutfile =  new TFile(Form("~/rootfiles/ana_run_%d.root",argc),"RECREATE");
		//rootoutfile->cd();
		TTree* Tout = new TTree("Tout","QDC_GEM_data");
	       //Branch of output root file
	       Tout->Branch("NoOfDet",&NoOfDet,"NoOfDet/I");
       	       Tout->Branch("qdcchargeDN",&qdcchargeDN,"qdcchargeDN/D");
       	       Tout->Branch("qdcchargeUP",&qdcchargeUP,"qdcchargeUP/D");
       	       //Tout->Branch("xposition","std::vector < float >",&xposition);
	       //Tout->Branch("yposition","std::vector < float >",&yposition);
	       Tout->Branch("xposition",xposition,"xposition[NoOfDet]/D");
       	       Tout->Branch("yposition",yposition,"yposition[NoOfDet]/D");
       }
	//cout<<"test 2"<<endl;
	...
	for(int ij=0;ij<entries;ij++)
	{
		...
		for(int detnumber=0;detnumber<NDet;detnumber++)
		{
			xposition[detnumber]=0;
			yposition[detnumber]=0;
		}
		//xposition.clear();
		//yposition.clear();
		...
		for(int detnumber=0;detnumber<NDet;detnumber++)
		{
			...
			xposition[detnumber]=xcentroid1;
			...
			yposition[detnumber]=ycentroid1;
			...
			vector<double>().swap(xcmn);
    			...
    			for(size_t pqrst=0;pqrst<hitx.size();pqrst++)
			{
				hitx.clear();
			}
    			for(size_t pqrst=0;pqrst<hity.size();pqrst++)
			{
				hity.clear();
			}
    			for(size_t pqrst=0;pqrst<hitxgated.size();pqrst++)
			{
				hitxgated.clear();
			}
    			for(size_t pqrst=0;pqrst<hitygated.size();pqrst++)
			{
				hitygated.clear();
			}
		...
		//cout<<"event "<<ij<<"  filling rootfile  with QDC charge "<<qdccharge<<endl;
		//if(qdccharge>155 && xposition[0]>0 && xposition[1]>0 && xposition[2]>0 && yposition[0]> 0 && yposition[1]>0 && yposition[2]>0)
		if(isOut)
		{
			//Tout->Fill();
		}
